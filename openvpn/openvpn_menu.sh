...
case $CHOICE in
    ...
    "󰒄 IMT VPN")
        # Connect to Uni Passau VPN
        kitty -e bash -c "sudo openvpn --cd ~/.config/openvpn/ --config ~/.config/openvpn/imt/eduvpn.imt-atlantique.fr_internet_20250129_Rascoussier.ovpn --auth-nocache --auth-user-pass ~/.config/openvpn/imt/pass.txt; echo -e \"Waiting 100s...\"; sleep 100"
        ;;
    "󰒄 INSA Lyon VPN")
        # VPN INSA Lyon
